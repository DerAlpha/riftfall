...
export interface ProfileData {
  ...
  lastPlayedAt: number;
  /**
   * Movement unlocks granted outside the skill tree (dev console `unlock`); the M9 skill tree
   * grants them too (ProgressionApi.runBonuses).
   */
  ...
  // --- M9 meta progression (save v2; sanitized by save/sanitizeProgression.ts) ---
  progression: PlayerProgressData;
  skills: SkillTreeData;
  /** Per weapon id (defs/weapons WEAPON_IDS). */
  weaponProgress: Record<string, WeaponProgressData>;
  /** Per achievement id; only achievements with progress are stored. */
  achievements: Record<string, AchievementProgressData>;
  challenges: ChallengeData;
  cosmetics: CosmeticsData;
  lifetimeStats: LifetimeStatsData;
  /** Local leaderboards per `${mapId}:${modeId}` (best first). */
  leaderboards: Record<string, LeaderboardEntry[]>;
}

// ---------------------------------------------------------------------------
// Meta progression data (M9) – persisted inside ProfileData
// ---------------------------------------------------------------------------

export interface PlayerProgressData {
  /** 1..PROGRESSION.maxLevel. */
  level: number;
  /** XP into the current level (0 at the max level). */
  xp: number;
  /** 0..PROGRESSION.prestige.maxRank. */
  prestige: number;
  /** Highest level ever reached (skill points from levels survive a prestige). */
  highestLevel: number;
  /** Every XP point ever credited (statistics). */
  lifetimeXp: number;
}

export interface SkillTreeData {
  /** Node id → bought rank (1..maxRank); nodes without a rank are absent. */
  ranks: Record<string, number>;
  /** Respecs done so far (the first SKILL_RULES.respec.free are free). */
  respecs: number;
  /** Preferred start grenade / ability (must be unlocked); null = the map loadout's. */
  loadout: { grenade: string | null; ability: string | null };
}

export interface WeaponProgressData {
  /** 1..PROGRESSION.weapon.maxLevel. */
  level: number;
  /** XP into the current level (0 at the max level). */
  xp: number;
  /** Camo challenge counters (defs/progression WEAPON_COUNTERS ids: kills, headshots, …). */
  counters: Record<string, number>;
  /** Unlocked weapon-scope camo ids (defs/cosmetics CAMOS). */
  camos: string[];
  /** Equipped camo (weapon or global scope, must be unlocked); null = factory finish. */
  equippedCamo: string | null;
}

export interface AchievementProgressData {
  /** Best progress towards the target (count, best run value, distinct values). */
  progress: number;
  /** Unlock time (ms since epoch), 0 = locked. */
  unlockedAt: number;
  /** Distinct-value achievements: the values collected so far (lifetime scope). */
  seen?: string[];
}

export interface ChallengeSlotData {
  progress: number;
...
export interface ChallengeSetData {
  /** Rotation key (the date seed); '' = nothing generated yet. */
  key: string;
  /** Template ids of the generated set (a set generated by other defs resets its progress). */
  templates: string[];
  slots: ChallengeSlotData[];
}

export interface ChallengeData {
  daily: ChallengeSetData;
  weekly: ChallengeSetData;
  completedDaily: number;
  completedWeekly: number;
}

/** Equipped cosmetics; ids from defs/cosmetics.ts (camos are equipped per weapon). */
...
export interface CosmeticsData {
  /** "Rift-Splitter" (challenges, achievements, prestige): shop cosmetics and respecs. */
  currency: number;
  /** Unlocked cosmetic ids (charms, crosshairs, kill effects, emblems, global camos). */
  unlocked: string[];
  equipped: CosmeticLoadout;
}

export interface LifetimeStatsData {
  /** Totals keyed by defs/progression LIFETIME_COUNTERS ids (kills, deaths, shotsFired, …). */
  counters: Record<string, number>;
  /** Kills per enemy type / per weapon id. */
  killsByEnemy: Record<string, number>;
  killsByWeapon: Record<string, number>;
  /** Highest wave reached and best score per `${mapId}:${modeId}`. */
...
export interface LeaderboardEntry {
  ...
  kills: number;
  score: number;
  /** Points earned during the run. */
  points: number;
  /** Seconds survived. */
  time: number;
  /** Run end (ms since epoch). */
  date: number;
  /** Weapons carried at the end (ids). */
  weapons: string[];
  /** Perks owned at the end (ids). */
  perks: string[];
  /** Player level / prestige at the run end. */
  level: number;
  prestige: number;
  /** Seed of a seeded run (M8 daily challenge), null otherwise. */
  seed: string | null;
}
...
/** Why a skill node cannot be bought right now ('ok' = it can). */
export type SkillCheck = 'ok' | 'unknown' | 'maxed' | 'locked' | 'tier' | 'points';

/** Start-of-run effects of the skill tree beyond stat modifiers (the composition root applies them). */
export interface SkillRunBonuses {
  /** Extra start points. */
  startPoints: number;
  /** Extra grenades of the start type (capped by the type's maximum). */
  startGrenades: number;
  /** Fraction of every Rift-Kiste price paid back (0..1). */
  boxCashback: number;
  /** Movement abilities granted by the tree. */
  dash: boolean;
  doubleJump: boolean;
  /** Start grenade / ability types the player may choose (the defaults included). */
  grenades: readonly string[];
  abilities: readonly string[];
}

export interface SkillTreeApi {
  /** Points earned (levels + prestige ranks), spent on nodes, and left to spend. */
  readonly earned: number;
  readonly spent: number;
  readonly available: number;
  rank(nodeId: string): number;
  check(nodeId: string): SkillCheck;
  /** Buy the next rank of a node; false (nothing changed) unless check() is 'ok'. */
  unlock(nodeId: string): boolean;
  /** Refund every point (SKILL_RULES.respec: free respecs, then currency; never during a run). */
  respec(): boolean;
  /** Stat modifiers of every bought rank (source `skill:<nodeId>`). */
  modifiers(): StatModifier[];
  readonly bonuses: Readonly<SkillRunBonuses>;
}

/** A run as progression sees it (ProgressionApi.beginRun). */
export interface ProgressionRunInfo {
  mapId: string;
  /** Mode id (M8); RUN.defaultMode 'classic' until then. */
  mode: string;
  /** Seed of a seeded run (daily challenge mode), null otherwise. */
  seed: string | null;
  /** The run counts for XP, stats, achievements and leaderboards (wave maps; not the calibration hall). */
  ranked: boolean;
}
...
export interface WeaponProgressView {
  ...
  /** Camos usable on this weapon (weapon unlocks + global camos). */
  camos: readonly string[];
  equippedCamo: string | null;
}
...
export interface AchievementView {
  ...
  category: string;
  /** Hidden achievements show "???" until unlocked (the view still carries the real text). */
  hidden: boolean;
  progress: number;
  target: number;
  unlockedAt: number;
...
export interface ChallengeView {
  ...
  xp: number;
  currency: number;
  /** Cosmetic granted with the reward (weekly), null = none. */
  cosmetic: string | null;
  /** When this set rotates (ms since epoch). */
  endsAt: number;
}

export interface LifetimeStatsView {
  counters: Readonly<Record<string, number>>;
  /** 0..1 shots that hit / shots fired (0 without shots). */
  accuracy: number;
  /** Weapon with the most kills (null before the first kill). */
  favouriteWeapon: string | null;
  highestWave(mapId: string, mode?: string): number;
  killsByEnemy: Readonly<Record<string, number>>;
  killsByWeapon: Readonly<Record<string, number>>;
}
...
export interface ProgressionApi {
  ...
  /** Permanent XP multiplier of the prestige rank (1 = none). */
  readonly xpMultiplier: number;
  readonly skills: SkillTreeApi;
  /** Credit XP (× xpMultiplier); returns the amount credited. */
  addXp(amount: number, source: XpSource): number;
  canPrestige(): boolean;
  /** Level back to 1, rank + 1: keeps unlocks and skill points, grants the rank's emblem. */
  prestige(): boolean;
  weapon(weaponId: string): WeaponProgressView | null;
  achievements(): AchievementView[];
  /** Current daily + weekly challenges (rotated when the UTC day / week changed). */
  challenges(): ChallengeView[];
  /** Grant a completed challenge's reward (auto-claimed unless PROGRESSION_CHALLENGES.autoClaim is off). */
  claimChallenge(id: string): boolean;
  readonly stats: LifetimeStatsView;
  leaderboard(mapId: string, mode?: string): readonly LeaderboardEntry[];
  /** A run starts: per-run tracking resets (the previous run is committed if it never ended). */
  beginRun(info: ProgressionRunInfo): void;
  /** Re-apply the skill tree's stat modifiers (after StatsApi.reset). */
  applySkillModifiers(stats: Pick<StatsApi, 'addModifier' | 'removeSource'>): void;
  /** Write pending progress now (unload); no-op when nothing is pending. */
  flush(): void;
}
