...
describe('runFixedTick', () => {
  it('steps player → interaction → weapons → grenades → abilities → targets → waves → enemies → projectiles → fields → status → interactables → powerUps → physics → health → perks → level → runFlow', () => {
    const calls: string[] = [];
