...
void main() {
  ...
  if (style == 0) {
    ...
    float n = aFbm(q * 3.2 + seed * 3.0);
    float arms = 0.5 + 0.5 * sin(ang * 2.0 + log(max(r, 1e-3)) * 6.0 + t * 1.5);
    float heat = pow(1.0 - smoothstep(inner, 1.0, r), 1.6);
    float band = smoothstep(inner, inner + 0.05, r) * (1.0 - smoothstep(0.5, 1.0, r));
    a = band * (0.25 + n * 1.2) * (0.35 + 0.65 * arms) * (0.3 + heat * 1.8);
    a += exp(-pow((r - inner - 0.02) / 0.02, 2.0)) * 1.4;
    ...
    float bub = exp(-pow((length(f) - ph * 0.38) / 0.035, 2.0)) * (1.0 - ph) * step(0.5, h);
    float caustic = aRidge(aNoise(p * 6.0 + vec2(t * 0.35, -t * 0.25)), 6.0);
    a = puddle * (0.2 + 0.3 * n + caustic * 0.3 + bub * 1.6);
    core = puddle * bub * 0.4;
  } else {
    ...
    float veins = aRidge(aNoise(p * 7.0 + seed * 3.0), 9.0) * 0.7 + aRidge(aNoise(p * 15.0 - seed), 13.0) * 0.45;
