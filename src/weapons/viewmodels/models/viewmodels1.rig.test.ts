...
describe('C1 viewmodels in the rig', () => {
  ...
  it('full ADS puts each sight socket on the view axis at its eye distance', () => {
    for (const id of IDS) {
      rig.showWeapon(id);
  ...
  it('switches between them never sweep a weapon (stock, butt, magazine) up through the view', () => {
    const ray = new Raycaster();
    const ndc = new Vector2();
    const probes: [number, number][] = [];
    for (const x of [-0.9, -0.6, -0.3, 0, 0.3, 0.6, 0.9]) for (const y of [0.05, 0.35, 0.7]) probes.push([x, y]);
  ...
  it('reloads keep every weapon below the upper third of the screen', () => {
    const ray = new Raycaster();
    const ndc = new Vector2();
    for (const id of IDS) {
      rig.showWeapon(id);
      ...
      while (t < duration + 0.3) {
        ...
        for (const x of [-0.6, -0.3, 0, 0.3, 0.6]) {
          ...
          expect(ray.intersectObject(rig.root, true).length, `${id} reload t=${t.toFixed(2)} @ ${x}`).toBe(0);
        }
      }
